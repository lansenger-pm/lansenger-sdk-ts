...
const schedule = await client.createSchedule(
  ...
  [{ staffId: "staff1", attendeeFlag: "yes" }], // 可選值 yes/option/no — 伺服器不接受 "required"
  { user_token: "ut" },
);
...
| `calendar *` | N | **Y** | **Y** | 有 userToken = 使用者身分；無 = 機器人身分 |
| `todo *` | N | **Y** | **Y** | 僅限組織級應用 |
...
>
> **組織應用 vs 組織應用+機器人**：相同的 appID/appSecret。唯一區別在於訊息通道——只有機器人可以傳送機器人私聊和群聊訊息（因為只有機器人能入群）。其他所有 API（通訊錄、日曆、待辦、聊天、OAuth2、串流）兩者完全一樣。目前只有自建應用支援機器人能力。
...
| 群組唯讀 | 對群組的唯讀存取 | `lansenger-group`（查詢資訊/成員） |
| 群組編輯 | 對群組的編輯存取 | `lansenger-group`（建立/更新/解散/成員） |
| 日曆唯讀 | 對日曆日程的唯讀存取 | `lansenger-calendar`（查詢） |
| 日曆編輯 | 對日曆日程的編輯存取 | `lansenger-calendar`（建立/更新/刪除） |
| 上傳媒體 | 上傳媒體檔案權限 | `lansenger-media`（上傳、應用上傳） |
| 工作台範本讀 | 對工作台範本的唯讀存取 | — |
| 工作台範本寫 | 對工作台範本的寫入存取 | — |
