...
const schedule = await client.createSchedule(
  ...
  [{ staffId: "staff1", attendeeFlag: "yes" }], // 可選值 yes/option/no — 服務器不接受 "required"
  { user_token: "ut" },
);
...
| `department *` | N | **Y** | **Y** | 僅限組織級應用 |
| `calendar *` | N | **Y** | **Y** | 有 userToken = 使用者身分；無 = 機械人身分 |
| `todo *` | N | **Y** | **Y** | 僅限組織級應用 |
| `chat list/messages` | N | **Y** | **Y** | 僅限組織級應用 |
| `group *` (羣組管理 V2) | N | N | **Y** | 需要機械人已在羣內 |
| `media upload` | **Y** | **Y** | **Y** | 通用上載 |
| `media upload-app` | **Y** | **Y** | **Y** | 僅限自建應用（非 ISV） |
| `media download/path` | **Y** | **Y** | **Y** | 通用下載 |
| `oauth *` | N | **Y** | **Y** | 僅限組織級應用 |
| `streaming *` | N | **Y** | **Y** | 僅限組織級應用 |
| `callback *` (事件解析) | N/A | N/A | N/A | 純資料操作，不需要身分 |


> **個人機械人**只能收發訊息和上載/下載檔案。無法存取通訊錄、日曆或 OAuth2。
>
> **組織應用 vs 組織應用+機械人**：相同的 appID/appSecret。唯一區別在於訊息通道——只有機械人可以傳送機械人私聊和羣聊訊息（因為只有機械人能入羣）。其他所有 API（通訊錄、日曆、待辦、聊天、OAuth2、串流）兩者完全一樣。目前只有自建應用支援機械人能力。

### 開發者中心權限

除身分類型外，特定 API 呼叫還取決於藍信開發者中心的權限開關。組織可能會限制開發者存取，需要管理員協助。

**基礎權限（預設開啟）：**

| 權限 | 說明 |
|------|------|
| 取得使用者基本資訊 | 取得使用者基本資訊用於系統/工作台登入 |
| 傳送通知訊息 | 取得組織訊息通道向使用者/羣組傳送訊息 |

**進階權限（預設關閉，需手動開啟）：**

| 權限 | 說明 | 影響的功能模組 |
|------|------|-------------|
| 通訊錄唯讀 | 對通訊錄的唯讀存取 | `lansenger-staff`、`lansenger-department` |
| 通訊錄編輯 | 對通訊錄的編輯存取 | `lansenger-staff`（建立/更新/刪除） |
| 敏感資訊-手機 | 存取使用者手機號碼 | `lansenger-staff`（詳情、ID 映射） |
| 敏感資訊-電子郵件 | 存取使用者電子郵件 | `lansenger-staff`（詳情、ID 映射） |
| 敏感資訊-身分證號 | 存取使用者身分證號 | `lansenger-staff` |
| 敏感資訊-員工編號 | 存取使用者員工編號 | `lansenger-staff` |
| 映射唯一標識到員工 ID | 手機/電子郵件/員工編號映射到員工 ID | `lansenger-staff`（ID 映射） |
| 應用編輯 | 建立和更新應用 | 開發者中心管理 |
| 羣組唯讀 | 對羣組的唯讀存取 | `lansenger-group`（查詢資訊/成員） |
| 羣組編輯 | 對羣組的編輯存取 | `lansenger-group`（建立/更新/解散/成員） |
| 日曆唯讀 | 對日曆日程的唯讀存取 | `lansenger-calendar`（查詢） |
| 日曆編輯 | 對日曆日程的編輯存取 | `lansenger-calendar`（建立/更新/刪除） |
| 上載媒體 | 上載媒體檔案權限 | `lansenger-media`（上載、應用上載） |
| 工作台範本讀 | 對工作台範本的唯讀存取 | — |
| 工作台範本寫 | 對工作台範本的寫入存取 | — |

遇到權限錯誤時，請先確認身分類型是否支援該操作，然後提示使用者在開發者中心開啟對應的進階權限（無法存取請聯絡組織管理員）。
...
│   ├── boardrooms.ts        # 會議室預訂 V2
│   ├── personalTodos.ts     # 個人待辦
│   ├── calendars.ts          # 日曆日程
