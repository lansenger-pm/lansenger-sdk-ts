...
除身份类型外，特定 API 调用还取决于蓝信开发者中心的权限开关。组织可能会限制开发者访问，需要管理员协助。

**基础权限（默认开启）：**

| 权限 | 说明 |
|------|------|
| 获取用户基本信息 | 获取用户基本信息用于系统/工作台登录 |
| 发送通知消息 | 获取组织消息通道向用户/群组发送消息 |

**高级权限（默认关闭，需手动开启）：**

| 权限 | 说明 | 影响的功能模块 |
|------|------|-------------|
| 通讯录只读 | 对通讯录的只读访问 | `lansenger-staff`、`lansenger-department` |
| 通讯录编辑 | 对通讯录的编辑访问 | `lansenger-staff`（创建/更新/删除） |
| 敏感信息-手机 | 访问用户手机号 | `lansenger-staff`（详情、ID 映射） |
| 敏感信息-邮箱 | 访问用户邮箱 | `lansenger-staff`（详情、ID 映射） |
| 敏感信息-身份证号 | 访问用户身份证号 | `lansenger-staff` |
| 敏感信息-工号 | 访问用户工号 | `lansenger-staff` |
| 映射唯一标识到员工 ID | 手机/邮箱/工号映射到员工 ID | `lansenger-staff`（ID 映射） |
| 应用编辑 | 创建和更新应用 | 开发者中心管理 |
| 群组只读 | 对群组的只读访问 | `lansenger-group`（查询信息/成员） |
| 群组编辑 | 对群组的编辑访问 | `lansenger-group`（创建/更新/解散/成员） |
| 日历只读 | 对日历日程的只读访问 | `lansenger-calendar`（查询） |
| 日历编辑 | 对日历日程的编辑访问 | `lansenger-calendar`（创建/更新/删除） |
| 上传媒体 | 上传媒体文件权限 | `lansenger-media`（上传、应用上传） |
| 工作台模版读 | 对工作台模板的只读访问 | — |
| 工作台模版写 | 对工作台模板的写入访问 | — |

遇到权限错误时，请先确认身份类型是否支持该操作，然后提示用户在开发者中心开启相应的高级权限（无法访问请联系组织管理员）。
...
│   ├── todos.ts              # 统一待办
│   ├── notices.ts           # 通知系统
│   ├── questionnaires.ts    # 问卷系统
│   ├── boardrooms.ts        # 会议室预定 V2
│   ├── personalTodos.ts     # 个人待办
│   ├── calendars.ts          # 日历与日程
